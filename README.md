...
> 仓库默认**不包含个人词库**，只保留 `data/standard_template.md` 作为格式示例。

只需三步：

1. 在 `data/` 下新建一个**文件夹**，文件夹名即为牌组名（支持中文），例如：
   ```
   data/我的单词本/
   ```
2. 在文件夹内放入任意 `.md` 文件，按模板格式填写词条：
   ```
   data/我的单词本/words.md
   ```
3. 启动程序后按 `s` 同步，单词本自动出现在侧边栏。

可参考 `data/standard_template.md` 了解完整格式说明。
...
程序以**第一列词条**为全局唯一键（不区分牌组）。如果同一个英文词在多个地方出现但含义不同，第二次及之后的同名词条会被**自动跳过**，不会报错也不会覆盖原有记录。

**建议写法**：遇到同词多义时，在词条上加以区分，例如：

| 英文词汇 | 中文释义 | 备注 / 例句 |
| --- | --- | --- |
| **lean** | adj. 瘦的 | lean pork 瘦肉 |
| **lean back** | v. 往后仰；倾斜 | lean back in the chair 靠在椅背上 |
